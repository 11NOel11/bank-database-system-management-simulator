...
# Main function runs all the queries by categorizing into two types
...
# For security of bank account, customizable encryption logic
def encrypt_password(password, key=11):
    encrypted_password = ''
    for char in password:
        encrypted_password += chr((ord(char) + key) % 128) 
    return encrypted_password
#account creation you can ofc change the key provided throughout or make it random geeration
...
def main_investments_menu(connection, account_number):
    while True:
        ...
        if choice == '1':
            ...
            investment_status(connection, account_number)  # Call investment_status
        elif choice == '3':
            break
        else:
            print("Invalid choice. Please choose a valid option.")
...
def investment_status(connection, account_number):
    query = "SELECT * FROM investments WHERE account_number = %s;"
    data = (account_number,)
    cursor = connection.cursor()
    ...
    try:
        ...
        if result:
            for row in result:
                investment_id, account_number, investment_amount, return_rate, status = row
            ...
            print("No active investments for this account.")
...
def calculate_total_loanpaid(connection, loan_id):
    ...
    try:
        ...
        print(f"Error calculating total loan paid: {e}")
        return 0
...
def pay_interest(connection, loan_amount, interest_rate, account_number, current_balance):
    ...
    if current_balance >= upcoming_interest:
        ...
        query_update_balance = "UPDATE accounts SET balance = %s WHERE account_number = %s;"
        data_update_balance = (balance, account_number)
        execute_query(connection, query_update_balance, data_update_balance)
